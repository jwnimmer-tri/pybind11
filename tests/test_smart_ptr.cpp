...
TEST_SUBMODULE(smart_ptr, m) {
    ...
    public:
        ...
        UniquePtrHeld(int value) : value_(value) { print_created(this, value); }
        ~UniquePtrHeld() { print_destroyed(this); }
    ...
    py::class_<UniquePtrOther>(m, "UniquePtrOther").def(py::init<>());

    m.def("unique_ptr_pass_through", [](std::unique_ptr<UniquePtrHeld> obj) { return obj; });
    m.def("unique_ptr_terminal", [](std::unique_ptr<UniquePtrHeld> obj) {
        obj.reset();
        return nullptr;
    });

    // Check traits in a concise manner.
    static_assert(py::detail::move_common<std::unique_ptr<UniquePtrHeld>>::value,
                  "This must be true.");

    // Guarantee API works as expected.
    m.def("unique_ptr_pass_through_cast_from_py", [](py::object obj_py) {
        auto obj = py::cast<std::unique_ptr<UniquePtrHeld>>(std::move(obj_py));
        return obj;
    });
    m.def("unique_ptr_pass_through_move_from_py", [](py::object obj_py) {
        return py::move<std::unique_ptr<UniquePtrHeld>>(std::move(obj_py));
    });

    m.def("unique_ptr_pass_through_move_to_py",
          [](std::unique_ptr<UniquePtrHeld> obj) { return py::move(std::move(obj)); });

    m.def("unique_ptr_pass_through_cast_to_py",
          [](std::unique_ptr<UniquePtrHeld> obj) { return py::cast(std::move(obj)); });

    class FirstT {};
    py::class_<FirstT>(m, "FirstT").def(py::init());
    class SecondT {};
    py::class_<SecondT>(m, "SecondT").def(py::init());

    m.def("unique_ptr_overload", [](std::unique_ptr<UniquePtrHeld> obj, FirstT) {
        py::dict out;
        out["obj"] = py::cast(std::move(obj));
        out["overload"] = 1;
        return out;
    });
    m.def("unique_ptr_overload", [](std::unique_ptr<UniquePtrHeld> obj, SecondT) {
        py::dict out;
        out["obj"] = py::cast(std::move(obj));
        out["overload"] = 2;
        return out;
    ...
    m.def("shared_ptr_held_func",
          [](std::shared_ptr<SharedPtrHeld> obj) { return obj != nullptr && obj->value == 10; });
    ...
    struct UniquePtrHeldContainer {
        UniquePtrHeldContainer() { value_.reset(new UniquePtrHeld(10)); }
        UniquePtrHeld *get() const { return value_.get(); }
